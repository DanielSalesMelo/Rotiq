...
const ChatWidget: React.FC = () => {
  ...
  return (
    <div style={{ position: 'fixed', bottom: '20px', right: '20px', zIndex: 1000 }}>
      {!isOpen && (<button onClick={toggleChat} style={styles.floatingButton}>🤖</button>)}
      {isOpen && (
        <div style={styles.chatWindow}>
          <div style={styles.chatHeader}><span>Corey (Assistente IA)</span><button onClick={toggleChat} style={styles.closeButton}>×</button></div>
          <div style={styles.messagesContainer}>
            {messages.map((msg, index) => (<div key={index} style={msg.sender === 'user' ? styles.userMessage : styles.coreyMessage}>{msg.text}</div>))}
            {isLoading && <div style={styles.coreyMessage}>Digitando...</div>}
            <div ref={messagesEndRef} />
          </div>
          <div style={styles.inputContainer}><input type="text" value={input} onChange={(e) => setInput(e.target.value)} onKeyPress={(e) => e.key === 'Enter' && handleSend()} placeholder="Pergunte algo ao Corey..." style={styles.input} disabled={isLoading} /><button onClick={handleSend} style={styles.sendButton} disabled={isLoading}>Enviar</button></div>
        </div>
      )}
    </div>
  );
